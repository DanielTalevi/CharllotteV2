 ...
 * Formata as datas
 */
        const formatar = new Intl.DateTimeFormat(
            "pt-BR",
            {
                day: "2-digit",
                month: "2-digit",
        ...
        const inicio = formatar.format(menorData);
        const fim = formatar.format(maiorData);
        ...
        if (dias === 1) {
            ...
            elementoPeriodo.innerHTML =
                `${inicio}<small>1 dia</small>`;

        } else {

            elementoPeriodo.innerHTML =
                `${inicio} — ${fim}<small>${dias} dias</small>`;
